...
Paste plain text, BibTeX, RIS, or CSL-JSON in the **Input** panel, or use **Import** (toolbar or Ctrl+I) for `.bib`, `.ris`, `.json`, `.docx`, or `.pdf` (PDF ingest is accelerated 10–20× using the **Maktab Tier A Rust WASM Engine**).
</Step>
<Step title="Resolve identifiers and webpage metadata">
Use the top row in Input to paste a DOI, PMID, URL, or platform link (GitHub, Kaggle, Hugging Face, Substack, Medium, YouTube), then **Resolve** or click **Fetch webpage metadata** in Raqim Resolve to extract Open Graph, Dublin Core, and Schema.org metadata plus Wayback Machine archive fallbacks.
</Step>
...
<Figure src="/media/ui-references-menu.png" alt="References menu with Autocorrect, Find Missing DOIs, Verify references, Detect Duplicates, and Select Citation Style" caption="The References menu mirrors the toolbar actions and their shortcuts." />
...
3. Sanad 9B is the only tier — pick your quant (Q2_K–Q8_0) from the [Hugging Face repo](https://huggingface.co/QinEmPeRoR93/nassila-sanad-9b).
...
<Figure src="/media/ui-settings.png" alt="Settings dialog with notification, predatory journal list, and Manuscript source fetch options" caption="Settings → General — the Unpaywall email lives under Manuscript source fetch." />
...
**Per-reference PDF attach** is live (**1.3.0** / **1.6.0**): attach from a finding or bibliography row, then re-audit that reference. Open-access PDF text from Unpaywall is also used when available (**Masdar-lite**, **1.2.0**). Folder-scan bulk attach shipped in **1.10.0 Masdar Papers**.
...
- **Shahid** (tables/figures) and **Maktab** (native PDF inspection + scan fallback) are live in **2.0.0** alongside Masdar attach and Sanad passage grounding.
